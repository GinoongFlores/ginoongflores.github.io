    ...
    const navbarlinksActive = () => {
      ...
      NavLinks.forEach((navlink) => {
        ...
        if (position >= section.offsetTop && position <= section.offsetTop + section.offsetHeight) {
          document.querySelector(`.${navlink.link}-link`).classList.add('active-link');
        } else {
          document.querySelector(`.${navlink.link}-link`).classList.remove('active-link');
        }
      });
    };
  ...
  return (
    <>
      <nav
        ...
        <h1 className="w-full text-2xl font-bold invisible md:visible">@ginoongflores</h1>
        ...
        <div
          ...
          <h1 className="w-full text-2xl font-bold m-4 text-center text-white">ginoongflores</h1>
          {mapLinksResponsive}
        </div>
      </nav>
      <div>
